...
(defun cmake-build--invoke-build-current (&optional sentinel)
  (cmake-build--save-project-root ()
    (let* ((config (cmake-build--get-build-config))
           (command (concat "cmake --build " (cmake-build--get-build-dir) " "  cmake-build-options " --target " (car config)))
...
(defun cmake-build-run-cmake ()
  ...
  (let* ((default-directory (cmake-build--get-build-dir))
    ...
    (async-shell-command (concat "cmake -B" (cmake-build--get-build-dir)) buffer-name)
    (with-current-buffer buffer-name
      (visual-line-mode t))))
...
(defun cmake-build-clear-cache-and-configure ()
  ...
  (let* ((buffer-name (cmake-build--build-buffer-name))
         (command (concat "cmake " (cmake-build--get-cmake-options)
                          " -B" (cmake-build--get-build-dir)
...
(defun cmake-build-clean ()
  ...
  (let* ((default-directory (cmake-build--get-build-dir))
    ...
    (save-some-buffers t)
    (cmake-build--split-to-buffer buffer-name (cmake-build--run-buffer-name))
    (async-shell-command (concat "cmake --build " (cmake-build--get-build-dir) " --target clean") buffer-name)))
...
(defun cmake-build-other-target (target-name)
  ...
  (let* ((default-directory (cmake-build--get-build-dir))
    ...
    (save-some-buffers t)
    (cmake-build--split-to-buffer buffer-name (cmake-build--run-buffer-name))
    (async-shell-command (concat "cmake --build " (cmake-build--get-build-dir) " --target " target-name) buffer-name)))
